...
Code for identifing sex-specific risk markers in patiens with NSTEMI
"""
...
def data_scaler(data,path_model):
    ...
    scaler = pickle.load(open(path_model+"scaler_nstemi",'rb')) #load scaler
...
def specific_interactions_nstemi_women(xgb_model, data,path_model):
    ...
    data = data.rename(columns={'Avg. urea':'Avg. urea mg/dL',
                                ...
                                'Min. troponin T':'Min. troponin T ng/L',
                                'Avg. systolic blood pressure':'Avg. systolic blood pressure mmHg',
                                'Avg. heart rate':'Avg. heart rate bpm',
                                'Min. heart rate':'Min. heart rate bpm',
                                'Min. mean blood pressure':'Min. mean blood pressure mmHg',
                                'Min. white blood cells':'Min. white blood cells K/uL',
                                'Max. pap diastolic':'Max. pap diastolic mmHg',
                                'Avg. anion gap':'Avg. anion gap mEq/L',
                                })
    ...
    shap.dependence_plot(feature_list.index('Min. troponin T ng/L'), 
                         shap_values, xtrain_women, 
                         interaction_index=feature_list.index('Min. troponin T ng/L'),
    ...
    shap.dependence_plot(feature_list.index('Avg. creatinine umol/L'), 
                         shap_values, xtrain_women, 
                         interaction_index=feature_list.index('Min. troponin T ng/L'),
                         feature_names=feature_list)

    shap.dependence_plot(feature_list.index('Age'), shap_values, xtrain_women, 
                        interaction_index=feature_list.index('Avg. anion gap mEq/L'),
                        feature_names=xtrain_women.columns)

    shap.dependence_plot(feature_list.index('Age'), shap_values, xtrain_women, 
                        interaction_index=feature_list.index('Min. mean blood pressure mmHg'),
                        feature_names=xtrain_women.columns)

    shap.dependence_plot(feature_list.index('Min. troponin T ng/L'), shap_values, xtrain_women, 
...
def specific_interactions_nstemi_men(xgb_model, data,path_model):
    explainer = shap.TreeExplainer(xgb_model)
    data = data.rename(columns={'Avg. urea':'Avg. urea mg/dL',
                                'Max. creatinine':'Max. creatinine umol/L',
                                'Avg. creatinine':'Avg. creatinine umol/L',
    ...
    shap.dependence_plot(feature_list.index('Age'), shap_values, xtrain_men, 
                        interaction_index=feature_list.index('Avg. anion gap mEq/L'),
    ...
    shap.dependence_plot(feature_list.index('Min. troponin T ng/L'), shap_values, xtrain_men, 
                        interaction_index=feature_list.index('Age'),
                        feature_names=xtrain_men.columns)
    plt.show()

def specific_interactions_age(xgb_model, data,path_model):
    xtrain_women = data[(data.Sex==1)]
    xtrain_women_scaled=data_scaler(xtrain_women,path_model)
    feature_list=xtrain_women.columns.tolist()
    shap_interaction_values = shap.TreeExplainer(xgb_model).shap_interaction_values(xtrain_women_scaled)
    shap.dependence_plot((feature_list.index('age'), feature_list.index('age')),
                          shap_interaction_values, xtrain_women)
    plt.show()
...
def mortality_model(xtrain,xtest,ytrain,ytest, 
    ...
    specific_interactions_nstemi_men(model_loaded, join_sets,path_model)
    specific_interactions_nstemi_women(model_loaded, join_sets,path_model)
