...
drop view proce_nstemi cascade;
create view proce_nstemi as
select
  ids.subject_id
  ,ids.hadm_id
  ,ids.icustay_id
...
\copy (SELECT * FROM proce_nstemi) to '/tmp/proce_nstemi.csv' CSV HEADER;


drop view endotrach_nstemi;
create view endotrach_nstemi as
select
	ids.subject_id,
	ids.hadm_id,
	ids.icustay_id,
...
order by ids.subject_id,ids.hadm_id,ids.icustay_id;
\copy (SELECT * FROM endotrach_nstemi) to '/tmp/endotrach_nstemi.csv' CSV HEADER;