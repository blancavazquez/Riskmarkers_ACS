...
-- -------------------------------------------------------------------------
-- ------------------------------------------------------------------
-- Title: Extraction of arterial blood gas
...
--- Extraction of arterial blood gas
drop view blood_nstemi cascade;
create view blood_nstemi as
select
  e.subject_id
  ,e.hadm_id
  ,e.icustay_id
...
drop view blood_nstemi2 cascade;
create view blood_nstemi2 as
select
  e.subject_id
  ,e.hadm_id
  ,e.icustay_id
...
\copy (SELECT * FROM blood_nstemi2) to '/tmp/blood_nstemi2.csv' CSV HEADER;
