...
-- -------------------------------------------------------------------------
-- Title: Extraction of hemodinamia
...
drop view hemo_stemi cascade;
create view hemo_stemi as
select
  e.subject_id
  ,e.hadm_id
  ,e.icustay_id
...
, avg(case when ce.itemid in (420,220069,6166) then valuenum else null end) as lap_avg --Left Artrial Pressure --no results for nnstemi--no results for ccu
...
, avg(case when ce.itemid in (226272) then valuenum else null end) as ef_cco_avg --EF (CCO) --no results for nnstemi--no results for ccu
...
----------------- complete records
drop view hemo_stemi2 cascade;
create view hemo_stemi2 as
select
  ce.subject_id
  ,ce.hadm_id
  ,ce.icustay_id
  ...
  ,e.lap_min --Left Artrial Pressure --no results for nnstemi--no results for ccu
  ,e.lap_max --Left Artrial Pressure --no results for nnstemi--no results for ccu
  ,e.lap_avg --Left Artrial Pressure --no results for nnstemi--no results for ccu
  ...
  ,e.ef_cco_min --EF (CCO) --no results for nnstemi--no results for ccu
  ,e.ef_cco_max --EF (CCO) --no results for nnstemi--no results for ccu
  ,e.ef_cco_avg --EF (CCO) --no results for nnstemi--no results for ccu
  ,e.pcwp_min --PCWP
  ,e.pcwp_max --PCWP
  ,e.pcwp_avg --PCWP
  ,e.svO2_min --SvO2
...
\copy (SELECT * FROM hemo_stemi2) to '/tmp/hemo_stemi2.csv' CSV HEADER;
