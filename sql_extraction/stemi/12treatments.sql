...
drop view stemi_treat cascade;
create view stemi_treat as
select
  ids.subject_id
  ,ids.hadm_id
  ,ids.icustay_id
...
drop view stemi_treat2;
create view stemi_treat2 as
...
from stemi_ccu ids
left join stemi_treat pre
on ids.subject_id = pre.subject_id and ids.hadm_id = pre.hadm_id and ids.icustay_id = pre.icustay_id
where ids.los > 1.0
order by ids.subject_id;

\copy (SELECT * FROM stemi_treat2) to '/tmp/stemi_treat2.csv' CSV HEADER;
